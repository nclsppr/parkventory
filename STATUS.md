...
## Release SEO et i18n — fusionnée et déployée

- La [PR #29](https://github.com/nclsppr/parkventory/pull/29) a intégré
  `origin/main@45796acab60dbae79f406994074d3e8d8fab8824`, y compris les nouvelles
  interfaces godmode et administration d’organisation, puis a été fusionnée
  sur `main` au SHA `0d991742bf42dc9fc5d4ced0a0cedbb3652f10d7`.
- Les surfaces publiques, applicatives et administratives sont proposées en
  français, anglais, allemand et luxembourgeois. Les douze pages publiques ont
  des URLs stables, un canonical auto-référent, les alternates `fr`, `en`, `de`,
  `lb` et `x-default`, ainsi qu’un contenu localisé dans le HTML initial. Les
  routes privées, les callbacks et les 404 restent hors index.
- La racine négocie le cookie puis `Accept-Language`. Avant connexion, le choix
  de langue reste disponible sur les surfaces publiques. Dès qu’une session est
  reconnue, la préférence enregistrée du compte restaure la route équivalente et
  le seul sélecteur restant se trouve dans le profil.
- La migration additive `0006_user_locale.sql` a été appliquée puis vérifiée
  d’abord sur `parkventory-preview`, ensuite sur `parkventory-production`, avant
  le déploiement. Les deux bases enregistrent `0001` à `0006`, sans migration en
  attente. En production, les deux comptes existants ont été conservés et
  aucune préférence invalide n’a été observée ; aucune donnée personnelle n’a
  été lue pendant ces contrôles.
- La gate locale complète a réussi avec 6 tests de marque, 1 test d’upgrade de
  migration, 191 tests Worker/D1 et 148 tests frontend, les typechecks, le build
  Vite, le contrôle des types Cloudflare et les dry-runs des deux environnements.
  La gate de PR `33404908642` et la gate post-fusion `33405080390` ont réussi sur
  leurs SHA exacts.
- Workers Builds a publié le déploiement
  `3264784b-09af-47d9-8782-6d68c5388a2f`, version
  `1f17485d-4a69-4b30-a96a-f28cf54ff061`, à 100 % du trafic. La santé répond
  `200`, `www` redirige en `308`, les douze URLs sont dans le sitemap, une route
  inexistante répond réellement `404`, et sept routes privées sondées exposent
  `X-Robots-Tag: noindex, nofollow` sans `hreflang`.
- Les SHA-256 des bundles principal, CSS et administration servis publiquement
  correspondent exactement au build validé. Le parcours authentifié réel et
  l’envoi d’un lien magique n’ont pas été actionnés pendant cette livraison.
- Le navigateur intégré reste refusé par une politique administrateur qui n’a
  pas été contournée. La preuve visuelle reste la matrice Chrome locale
  antérieure, complétée par les tests DOM automatisés ; aucune nouvelle preuve
  perceptive n’est revendiquée pour la production.

## Socle Cloudflare — snapshot historique du 26 août 2026
